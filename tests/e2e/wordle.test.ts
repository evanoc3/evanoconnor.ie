...
import { successfulNavigation, validateLinkTags, validateMetaTags, validateSitemap } from "#tests/utils/e2e-test-utils.ts";


test.describe("/playground/wordle/", () => {

  test.beforeEach(async ({ page }) => {
    await successfulNavigation(page, "/playground/wordle/");
  });
  ...
  test("has the correct metadata", async ({ page }) => {
    ...
    await validateLinkTags(page, {
      canonicalLinkValue: "/playground/wordle/$"
    });
  });
